...
Fetch.prototype.getCollectionJson = function queueAnAjaxRequestForEachOfTheCollectionsPagesAndBuildJsonContext(handle) {
  ...
  for (var i = 0; i < this.pageCount; i++) {
    ...
    request.url = this.options.resourceUrl +'?view=json&handle='+this.options.paginationLimit+'&page='+ pageNumber+this.options.collectionQuery;
    ...
    if (this.firstPage && pageNumber == 1) {
      request.url = this.options.resourceUrl + '?view=json&order=50&handle=' + this.options.paginationLimit + '&page=' + pageNumber + this.options.collectionQuery;
